<x-app-layout>
    ...
    <div class="min-h-screen bg-gray-50/50 dark:bg-gray-900 pb-12" x-data="editorPage({{ $page->id }})">
        ...
        <div
            class="sticky top-0 z-30 bg-white/80 dark:bg-gray-800/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-700 mb-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="h-16 flex items-center justify-between gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="togglePageStatus()"
                            class="hidden sm:inline-flex items-center px-4 py-2 border rounded-lg text-sm font-bold transition-colors"
                            :class="isPublic ?
                                'border-emerald-200 text-emerald-700 bg-emerald-50 dark:bg-emerald-900/30 dark:border-emerald-800 dark:text-emerald-400' :
                                'border-rose-200 text-rose-700 bg-rose-50 dark:bg-rose-900/30 dark:border-rose-800 dark:text-rose-400'">
                            <span class="w-2 h-2 rounded-full mr-2"
                                :class="isPublic ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'"></span>
                            <span x-text="isPublic ? 'Aktif' : 'Nonaktif'"></span>
                        </button>
                        <a href="{{ url($page->handle) }}" target="_blank"
                            class="hidden sm:inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-bold text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            Lihat
                        </a>
                        <button @click="copyToClipboard('{{ url($page->handle) }}')"
                            class="px-4 py-2 rounded-lg text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition shadow-lg shadow-emerald-600/20">
                            Salin Link
                        </button>
                    </div>
                </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)"
                    class="mb-6 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 p-4 rounded-xl flex justify-between items-center shadow-sm">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div class="lg:col-span-7 space-y-8">
                    ...
                    <div
                        class="bg-white dark:bg-gray-800 p-1.5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex">
                        <button @click="activeTab = 'links'"
                            :class="activeTab === 'links' ?
                                'bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white font-bold shadow-sm' :
                                'text-gray-500 hover:text-gray-700 dark:text-gray-400'"
                            class="flex-1 py-2.5 rounded-lg text-sm transition-all flex justify-center items-center gap-2">
                    ...
                    <div x-show="activeTab === 'links'" x-transition:enter="transition ease-out duration-200 opacity-0"
                        x-transition:enter-end="opacity-100">

                        <div class="mb-10">
                            <h3
                                class="flex items-center gap-2 text-sm font-bold text-gray-800 dark:text-white uppercase tracking-wide mb-3 pl-1">
                                <span class="w-2 h-2 rounded-full bg-emerald-500"></span> Tombol Utama
                            </h3>

                            <div
                                class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 mb-4">
                                <form @submit.prevent="addLink($event)"
                                    action="{{ route('page-links.store', $page->id) }}"
                                    ...
                                    <div class="flex-1 space-y-3 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-3">
                                        <input type="text" name="title" required placeholder="Judul (mis: Website)"
                                        ...
                                        <div class="relative w-full" x-data="urlAutocomplete('')"
                                            ...
                                            <div x-show="show" x-transition
                                                ...
                                                <div x-show="isLoading"
                                                    class="p-3 text-xs text-gray-500 text-center animate-pulse">Mencari
                                                    tautan...</div>
                                                <div x-show="!isLoading && results.length === 0"
                                                    class="p-3 text-xs text-gray-500 text-center">Tautan tidak
                                                    ditemukan.</div>
                                                <template x-for="item in results" :key="item.id">
                                                    <div @click="selectLink(item.short_url)"
                                                        class="p-3 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 cursor-pointer border-b border-gray-100 dark:border-gray-700 last:border-0 transition-colors">
                                                        <div class="text-sm font-bold text-gray-800 dark:text-gray-200 truncate"
                                                            x-text="item.title"></div>
                                                        <div class="text-xs text-emerald-600 dark:text-emerald-400 truncate"
                                                            x-text="item.short_url"></div>
                                                    </div>
                                                </template>
                                            </div>
                                        </div>
                                    ...
                                    <button type="submit"
                                        class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-4 py-2.5 rounded-xl shadow-lg transition text-sm whitespace-nowrap">
                            ...
                            <div class="space-y-3" x-ref="listButtons" id="button-list">
                                ...
                                @forelse($buttonLinks as $link)
                                    <div data-id="{{ $link->id }}" x-data="{ id: {{ $link->id }}, title: '{{ addslashes($link->title) }}', url: '{{ addslashes($link->destination_url) }}', active: {{ $link->is_active ? 1 : 0 }}, icon: '' }"
                                        ...
                                        <div class="flex items-center gap-4 flex-1 min-w-0">
                                            <div class="text-gray-300 group-hover:text-emerald-500 handle cursor-grab">
                                        ...
                                        <div class="flex items-center gap-1 ml-2">
                                            <button @click="openEditModal(id, title, url, active, '', 'button')"
                                                class="p-2 text-gray-400 hover:text-blue-600 bg-gray-50 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition"><svg
                                            ...
                                            <button @click="deleteLink(id)"
                                                class="p-2 text-gray-400 hover:text-red-600 bg-gray-50 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition">
                                ...
                                @empty
                                    <div
                                        class="text-center py-6 border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl text-gray-400 text-xs">
                                        Belum ada tombol utama.</div>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div
                                class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 mb-4">
                                <form @submit.prevent="addLink($event)"
                                    action="{{ route('page-links.store', $page->id) }}" class="flex flex-col gap-3">
                                    @csrf
                                    <input type="hidden" name="type" value="link">
                                    ...
                                    <div class="flex flex-col sm:flex-row gap-3">
                                        <div class="sm:w-1/3">
                                            <select name="icon" x-model="newSocialIcon"
                                                class="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-blue-500 dark:bg-gray-900 dark:border-gray-700 dark:text-white text-sm font-sans"
                                                required>
                                        ...
                                        <div class="flex-1">
                                            <div class="relative w-full" x-data="urlAutocomplete('')"
                                                ...
                                                <div x-show="show" x-transition
                                                    class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                                                    <div x-show="isLoading"
                                                        class="p-3 text-xs text-gray-500 text-center animate-pulse">
                                                        Mencari tautan...</div>
                                                    <div x-show="!isLoading && results.length === 0"
                                                        class="p-3 text-xs text-gray-500 text-center">Tautan tidak
                                                        ditemukan.</div>
                                    ...
                                    <div class="text-right">
                                        <button type="submit"
                                            class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded-xl shadow-lg shadow-blue-600/20 transition text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                                            :disabled="!newSocialIcon">
                            ...
                            <div class="space-y-3" x-ref="listSocials" id="social-list">
                                ...
                                @forelse($socialLinks as $link)
                                    <div data-id="{{ $link->id }}" x-data="{ id: {{ $link->id }}, title: '{{ addslashes($link->title) }}', url: '{{ addslashes($link->destination_url) }}', active: {{ $link->is_active ? 1 : 0 }}, icon: '{{ $link->settings['icon'] ?? '' }}' }"
                                        ...
                                        <div class="flex items-center gap-4 flex-1 min-w-0">
                                            <div class="text-gray-300 group-hover:text-blue-500 handle cursor-grab">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2" d="M4 8h16M4 16h16"></path>
                                        ...
                                        <div class="flex items-center gap-1 ml-2">
                                            <button @click="openEditModal(id, title, url, active, icon, 'social')"
                                                class="p-2 text-gray-400 hover:text-blue-600 bg-gray-50 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition"><svg
                                                    class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                        ...
                        <form action="{{ route('pages.update', $page->id) }}" method="POST"
                            ...
                            <div
                                ...
                                <div class="flex flex-col sm:flex-row items-center gap-6 mb-6">
                                    <div
                                        class="relative group cursor-pointer shrink-0 flex flex-col items-center gap-3">
                                        <div
                                            class="w-24 h-24 rounded-full overflow-hidden border-4 border-gray-100 dark:border-gray-700 group-hover:border-emerald-500 transition-colors relative bg-gray-50 dark:bg-gray-900">
                                            @if ($page->avatar_path)
                                                <img id="preview-avatar"
                                                    src="{{ asset('storage/' . $page->avatar_path) }}"
                                                    class="w-full h-full object-cover">
                                            @else
                                                <div id="default-avatar"
                                                    class="w-full h-full bg-emerald-100 dark:bg-emerald-900 flex items-center justify-center text-3xl font-bold text-emerald-600 dark:text-emerald-400 select-none">
                                                    @php
                                        ...
                                        <input type="file" name="avatar"
                                            class="absolute inset-0 opacity-0 cursor-pointer w-24 h-24"
                                        ...
                                        @if ($page->avatar_path)
                                            <div class="flex items-center gap-2">
                                                ...
                                                <label for="delete_avatar"
                                                    class="text-xs text-red-500 font-bold cursor-pointer hover:text-red-700">Hapus
                                                    Foto</label>
                                            </div>
                                        @endif
                                    ...
                                    <div class="flex-1 w-full space-y-4">
                                        ...
                                        <div>
                                            <label
                                                class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Judul
                                                Halaman</label>
                                        ...
                                        <div>
                                            <label
                                                class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Handle
                                                URL (Link)</label>
                                            <div
                                        ...
                                        <div>
                                            <label
                                                class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Bio
                                                Deskripsi</label>
                                            <textarea name="bio" rows="2"
                                                class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:bg-gray-900 dark:border-gray-700 dark:text-white text-sm"
                                                placeholder="Bio Deskripsi">{{ old('bio', $page->bio) }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                <h3
                                    class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wide mb-6 border-b border-gray-100 dark:border-gray-700 pb-2">
                                    Gambar Latar Belakang</h3>
                                <div class="relative group">
                                    <label
                                        class="relative block w-full h-32 rounded-xl border-2 border-dashed @error('background_image') border-red-500 @else border-gray-300 @enderror hover:border-emerald-500 bg-gray-50 flex flex-col items-center justify-center cursor-pointer overflow-hidden">
                                        <img id="background-preview"
                                            src="{{ isset($page->appearance['background_image_path']) ? asset('storage/' . $page->appearance['background_image_path']) : '' }}"
                                            class="{{ isset($page->appearance['background_image_path']) ? '' : 'hidden' }} absolute inset-0 w-full h-full object-cover z-10">
                                        <div id="background-placeholder"
                                            class="{{ isset($page->appearance['background_image_path']) ? 'hidden' : '' }} text-center z-20">
                                            <p class="text-xs text-gray-500">Klik untuk upload background</p>
                                        </div>
                                ...
                                @if (isset($page->appearance['background_image_path']) && $page->appearance['background_image_path'])
                                    <div class="mt-2 flex items-center"><input type="checkbox"
                                            name="delete_background" class="mr-2 rounded text-red-600"><label
                                            class="text-xs text-red-600">Hapus background</label></div>
                                @endif
                            </div>
                            ...
                            <button type="submit"
                                class="w-full bg-emerald-600 text-white font-bold py-3 rounded-xl shadow-lg">Simpan
                                Tampilan</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="hidden lg:block lg:col-span-5 sticky top-24">
                    <div
                        class="relative mx-auto border-gray-900 bg-gray-900 border-[12px] rounded-[3rem] h-[650px] w-[340px] shadow-2xl flex flex-col overflow-hidden">
                        <div class="absolute top-0 inset-x-0 h-6 bg-gray-900 z-20 flex justify-center">
                            <div class="h-4 w-24 bg-black rounded-b-xl"></div>
                        </div>
                        <div class="absolute top-10 right-4 z-30"><button @click="refreshPreview()"
                                class="p-1.5 bg-black/30 text-white rounded-full backdrop-blur-md hover:bg-black/50 transition"><svg
                                    class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15">
                                    </path>
                                </svg></button></div>
                        <iframe id="previewFrame" src="{{ url($page->handle) }}" class="w-full h-full border-0"
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden"
                ...
                <form @submit.prevent="updateLink()" class="p-6 space-y-4">
                    ...
                    <div x-show="editDisplayAs == 'social'">
                        ...
                        <select name="icon" x-model="editIcon"
                            class="w-full px-4 py-2 rounded-xl border border-gray-200 dark:bg-gray-900 dark:border-gray-700 dark:text-white text-sm font-sans">
                            <option value="" disabled>Pilih Sosmed...</option>
                            <option value="fa-brands fa-whatsapp">WhatsApp</option>
                            <option value="fa-brands fa-instagram">Instagram</option>
                            <option value="fa-brands fa-tiktok">TikTok</option>
                    ...
                    <div>
                        ...
                        <div class="relative w-full" x-data="urlAutocomplete('')" x-init="$watch('editUrl', value => query = value)"
                            ...
                            <div x-show="show" x-transition
                                class="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                                <div x-show="isLoading" class="p-3 text-xs text-gray-500 text-center animate-pulse">
                                    Mencari tautan...</div>
                                <template x-for="item in results" :key="item.id">
                                    <div @click="selectLink(item.short_url)"
                                        class="p-3 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 cursor-pointer border-b border-gray-100 dark:border-gray-700 last:border-0 transition-colors">
                    ...
                    <div class="flex items-center justify-between py-2">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" x-model="editIsActive" class="sr-only peer">
                            <div
                                class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer dark:bg-gray-700 peer-checked:bg-emerald-600 peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all">
                            </div>
                        </label>
                    </div>
                    ...
                    <div class="pt-4 flex justify-end gap-2">
                        ...
                        <button type="submit"
                            class="px-4 py-2 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-md disabled:opacity-50"
                            :disabled="isSaving">
    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('urlAutocomplete', (initialValue = '') => ({
                query: initialValue,
                results: [],
                show: false,
                isLoading: false,
                ...
                async fetchShortlinks() {
                    // Jangan mencari jika input terlalu pendek atau berupa URL lengkap
                    if (this.query.length < 2 || this.query.startsWith('http')) {
                        this.results = [];
                        this.show = false;
                        return;
                    }
                    ...
                    try {
                        const res = await fetch(
                            `/api/search-shortlinks?q=${encodeURIComponent(this.query)}`);
                ...
                selectLink(url) {
                    this.query = url;
                    this.show = false;
                    // Kirim event agar variabel di luar komponen (seperti editUrl) ikut berubah
                    this.$dispatch('url-selected', url);
                }
            }));
        });
            ...
            return {
                ...
                togglePageStatus() {
                    ...
                    fetch(`/pages/${pageId}`, {
                        ...
                        if (!response.ok) {
                            this.isPublic = !this.isPublic; // Kembalikan jika gagal
                            alert('Gagal mengubah status halaman.');
                        }
                    }).catch(err => {
                        this.isPublic = !this.isPublic; // Kembalikan jika error jaringan
                ...
                initSortable() {
                    ...
                    if (buttonsEl) Sortable.create(buttonsEl, {
                        animation: 150,
                        handle: '.handle',
                        ghostClass: 'bg-emerald-50',
                        onEnd: handleEnd
                    });
                    if (socialsEl) Sortable.create(socialsEl, {
                        animation: 150,
                        handle: '.handle',
