@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}">
        ...
        {{-- TAMPILAN MOBILE BARU (Keren & Sesuai Tema) --}}
        <div class="flex gap-2 items-center justify-between sm:hidden">
            
            {{-- Tombol Sebelumnya --}}
            @if ($paginator->onFirstPage())
                <span class="relative inline-flex flex-1 justify-center items-center px-4 py-2 text-sm font-medium text-gray-400 bg-gray-50 border border-gray-200 cursor-not-allowed rounded-xl dark:bg-gray-800/50 dark:border-gray-700 dark:text-gray-500 shadow-sm">
                    <i class="fa-solid fa-chevron-left mr-2 text-[10px]"></i> Sebelumnya
                </span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" class="relative inline-flex flex-1 justify-center items-center px-4 py-2 text-sm font-bold text-emerald-600 bg-white border border-emerald-200 rounded-xl hover:bg-emerald-50 hover:text-emerald-700 transition-all active:scale-95 shadow-sm dark:bg-gray-800 dark:border-emerald-900/50 dark:text-emerald-400 dark:hover:bg-gray-700">
                    <i class="fa-solid fa-chevron-left mr-2 text-[10px]"></i> Sebelumnya
                </a>
            @endif
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" class="relative inline-flex flex-1 justify-center items-center px-4 py-2 text-sm font-bold text-emerald-600 bg-white border border-emerald-200 rounded-xl hover:bg-emerald-50 hover:text-emerald-700 transition-all active:scale-95 shadow-sm dark:bg-gray-800 dark:border-emerald-900/50 dark:text-emerald-400 dark:hover:bg-gray-700">
                    Selanjutnya <i class="fa-solid fa-chevron-right ml-2 text-[10px]"></i>
                </a>
            @else
                <span class="relative inline-flex flex-1 justify-center items-center px-4 py-2 text-sm font-medium text-gray-400 bg-gray-50 border border-gray-200 cursor-not-allowed rounded-xl dark:bg-gray-800/50 dark:border-gray-700 dark:text-gray-500 shadow-sm">
                    Selanjutnya <i class="fa-solid fa-chevron-right ml-2 text-[10px]"></i>
