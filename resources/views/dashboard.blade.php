        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <div>
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Berikut adalah ringkasan performa akun Anda hari ini.</p>
                </div>
                <div class="flex gap-3">
                    <a href="{{ route('shortlinks.index') }}" class="px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 rounded-xl text-sm font-bold shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 transition flex items-center gap-2">
                        <i class="fa-solid fa-plus text-emerald-500"></i> Buat Link
                    </a>
                    <a href="{{ route('pages.index') }}" class="px-4 py-2 bg-emerald-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 transition flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 {{ $totalUsers > 0 ? 'md:grid-cols-2 lg:grid-cols-4' : 'md:grid-cols-3' }} gap-6">
                
                {{-- 1. CARD TOTAL KUNJUNGAN --}}
                <div class="relative overflow-hidden bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-6 shadow-lg text-white group hover:-translate-y-1 transition-transform duration-300">
                    <div class="absolute top-0 right-0 -mr-8 -mt-8 w-32 h-32 rounded-full bg-white/10 blur-2xl group-hover:bg-white/20 transition"></div>
                    <div class="relative z-10">
                        ...
                        <div class="mt-4 flex items-center text-xs text-emerald-100 bg-white/10 w-fit px-2 py-1 rounded-lg">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 hover:border-emerald-500/30 transition duration-300">
                    <div class="flex justify-between items-start">
                        <div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 hover:border-emerald-500/30 transition duration-300">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($totalPages) }}</h3>
                        </div>
                ...
                @if($totalUsers > 0)
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 hover:border-emerald-500/30 transition duration-300 border-l-4 border-l-orange-500">
                    <div class="flex justify-between items-start">
                        <div>
                        ...
                        <div class="p-3 bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 rounded-xl">
                            <i class="fa-solid fa-users text-xl"></i>
                        </div>
                    </div>
                    <div class="mt-4 flex items-center text-xs text-orange-600 bg-orange-50 dark:bg-orange-900/30 w-fit px-2 py-1 rounded-lg">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6"
                    ...
                    <div class="relative h-[300px] w-full group">
                        <div x-show="loading" class="absolute inset-0 flex items-center justify-center bg-white/50 dark:bg-gray-800/50 z-20 backdrop-blur-[1px] rounded-xl transition">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 flex flex-col">
                    ...
                    <div class="flex-1 overflow-y-auto pr-1 space-y-4">
                        @forelse($popularLinks as $link)
                            <div class="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-gray-700/30 hover:bg-emerald-50 dark:hover:bg-emerald-900/10 transition group">
                                <div class="flex items-center gap-3 overflow-hidden">
                                    <div class="w-8 h-8 rounded-lg bg-white dark:bg-gray-800 flex items-center justify-center text-emerald-500 shadow-sm text-xs font-bold border border-gray-100 dark:border-gray-600">
                                        {{ $loop->iteration }}
                                    </div>
                                    <div class="min-w-0">
                                        <p class="text-sm font-bold text-gray-900 dark:text-white truncate">{{ $link->title ?? 'Tanpa Judul' }}</p>
                                        <p class="text-xs text-gray-400 truncate">
                                            @if($link->page_id)
                                                <i class="fa-solid fa-pager text-[10px] mr-1"></i> Bio Link
                                            @else
                                                <i class="fa-solid fa-link text-[10px] mr-1"></i> /{{ $link->short_code }}
                                            @endif
                                        </p>
                                    </div>
                                </div>
                                <div class="text-right shrink-0">
                                    <span class="text-sm font-bold text-gray-800 dark:text-gray-200">{{ number_format($link->click_count) }}</span>
                    ...
                    <a href="{{ route('shortlinks.index') }}?sort=clicks&dir=desc" class="mt-4 text-center text-xs font-bold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 transition">
                        Lihat Semua &rarr;
                    </a>
                </div>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            @forelse($recentLinks as $link)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                    <td class="px-6 py-4">
                                        <div class="font-bold text-gray-900 dark:text-white">{{ $link->title ?? 'Tanpa Judul' }}</div>
                                        @if($link->page_id)
                                            <div class="text-purple-500 text-xs font-medium"><i class="fa-solid fa-pager mr-1"></i> Halaman Bio</div>
                                        @else
                                            <div class="text-emerald-600 text-xs font-medium">/{{ $link->short_code }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center text-gray-500 dark:text-gray-400 max-w-xs truncate gap-2">
                                            <img src="https://www.google.com/s2/favicons?domain={{ parse_url($link->destination_url, PHP_URL_HOST) }}&sz=16" class="w-4 h-4 opacity-70">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-8 text-center text-gray-400">Belum ada link yang dibuat.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        function analyticsHandler() {
            return {
                ...
                renderChart(shortlinkData, biolinkData) {
                    ...
                    var options = {
                        series: [
                            { name: 'Shortlink Visits', data: shortlinkData },
                            { name: 'Bio Link Visits', data: biolinkData }
                        ],
                        chart: {
                            type: 'area',
                            height: 300,
                        ...
                        xaxis: {
                            ...
                            labels: { 
                                style: { colors: '#9ca3af' },
                                datetimeFormatter: {
                                    year: 'yyyy', month: 'dd MMM', day: 'dd MMM', hour: 'HH:mm'
                                }
                            }
                        },
                        yaxis: {
